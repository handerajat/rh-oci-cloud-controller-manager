...
func (d BlockVolumeNodeDriver) NodeGetVolumeStats(ctx context.Context, req *csi.NodeGetVolumeStatsRequest) (*csi.NodeGetVolumeStatsResponse, error) {
	...
	isRawBlockVolume := false
	isDevice, err := hostUtil.PathIsDevice(volumePath)
	if err != nil {
	...
	if isDevice {
		...
		isDevice, err = hostUtil.PathIsDevice(csi_util.GetStagingTargetPathForBlock(volumePath))
		if err != nil {
		...
		if isDevice {
			isRawBlockVolume = true
			volumePath = csi_util.GetStagingTargetPathForBlock(volumePath)
		}
	}
