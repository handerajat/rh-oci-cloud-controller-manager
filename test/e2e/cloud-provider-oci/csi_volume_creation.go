...
var _ = Describe("CSI Raw Block Volume Creation", func() {
	...
	Context("[cloudprovider][storage][csi][system-tags][raw-block]", func() {
		...
		It("Create raw block PVC with VolumeSize 1Gi but should use default 50Gi", func() {
			...
			f.VolumeIds = append(f.VolumeIds, pvc.Spec.VolumeName)
			pvcJig.NewPodForCSIBlock("app2", f.Namespace.Name, pvc.Name, setupF.AdLabel)
		...
		It("Create raw block PVC with VolumeSize 100Gi should use 100Gi", func() {
			...
			f.VolumeIds = append(f.VolumeIds, pvc.Spec.VolumeName)
			pvcJig.NewPodForCSIBlock("app3", f.Namespace.Name, pvc.Name, setupF.AdLabel)
...
var _ = Describe("CSI Static Raw Block Volume Creation", func() {
	...
	Context("[cloudprovider][storage][csi][static][raw-block]", func() {
		It("Static Provisioning of a raw block CSI", func() {
			...
			pvcJig.NewPodForCSIBlock("app4", f.Namespace.Name, pvc.Name, setupF.AdLabel)
...
func TestCMEKAttachmentTypeAndEncryptionType(f *framework.CloudProviderFramework, expectedAttachmentType string, isRawBlockVolume bool) {
	...
	podName := ""
	if isRawBlockVolume {
		podName = pvcJig.NewPodForCSIBlock("app1", f.Namespace.Name, pvc.Name, setupF.AdLabel)
	} else {
		podName = pvcJig.NewPodForCSI("app1", f.Namespace.Name, pvc.Name, setupF.AdLabel)
	}
